...
from utils.utils import correctsqlqueryfunction
import os
...
def LaunchUI():
    ...
    st.write("Query {} DataBase".format(database))
    ...
    if user_message := st.chat_input("Enter your query?"):
        ...
        if MessagefromModel is None: 
            ...
            error,sql_results = query_database(FinalQuerry)
                
            if error == 1: sql_results = correctsqlqueryfunction(sql_results,FinalQuerry)
